...
namespace SceneInspector
{
    ...
    SceneData Parse(const uint8_t* data, size_t size)
    {
        ...
        if (!r.valid() || magic != MAGIC || version != VERSION)
            return sd;
    ...
    {
        if (CaseInsensitiveContains(g.nodes[idx].name, filter))
            return true;
        for (size_t ci : g.nodes[idx].children)
            if (HasFilterMatch(g, ci, filter))
                return true;
        return false;
    }
    ...
    void Inspector::RenderSceneExplorer(SceneData& data)
    {
        ...
        if (ImGui::TreeNode(hdr))
        {
            for (size_t i = 0; i < data.materials.size(); i++)
            {
                ...
                if (!CaseInsensitiveContains(mat.name, filter))
                    continue;
                char label[256];
        ...
        if (ImGui::TreeNode(hdr))
        {
            for (size_t i = 0; i < data.textures.size(); i++)
            {
                ...
                if (!CaseInsensitiveContains(tex.name, filter))
                    continue;
                char label[256];
        ...
        if (!data.animGroups.empty())
        {
            ...
            if (ImGui::TreeNode(hdr))
            {
                for (size_t i = 0; i < data.animGroups.size(); i++)
                {
                    ...
                    if (!CaseInsensitiveContains(ag.name, filter))
                        continue;
                    char label[256];
        ...
        if (!data.skeletons.empty())
        {
            ...
            if (ImGui::TreeNode(hdr))
            {
                for (size_t i = 0; i < data.skeletons.size(); i++)
                {
                    ...
                    if (!CaseInsensitiveContains(sk.name, filter))
                        continue;
                    char label[256];
    ...
    void Inspector::RenderCameraProps(CameraInfo& cam)
    {
        ...
        if (cam.isArcRotate && ImGui::CollapsingHeader("Arc Rotate###cp_arc", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            ImGui::PushItemWidth(-1);
            char aid[32];
            snprintf(aid, sizeof(aid), "###arc_a_%u", uid);
            ...
            ImGui::PushItemWidth(-1);
            char bid[32];
            snprintf(bid, sizeof(bid), "###arc_b_%u", uid);
    ...
    void Inspector::RenderTransformProps(float pos[3], float rot[4], float scl[3], bool hasQuat, uint32_t uid)
    {
        if (ImGui::CollapsingHeader("Transform###xf", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            char rid[64];
            snprintf(rid, sizeof(rid), "###rot_%u", uid);
            ImGui::Text("Rotation");
            ImGui::SameLine(100);
            ImGui::PushItemWidth(-1);
    ...
    void Inspector::RenderMeshProps(MeshInfo& mesh, const SceneData& data)
    {
        ...
        if (ImGui::CollapsingHeader("Display###mp_disp", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            char lmid[32];
            snprintf(lmid, sizeof(lmid), "###lm_%u", uid);
        ...
        if (!mesh.morphTargets.empty() && ImGui::CollapsingHeader("Morph Targets###mp_morph"))
        {
            for (size_t i = 0; i < mesh.morphTargets.size(); i++)
            {
                ...
                char mtid[64];
                snprintf(mtid, sizeof(mtid), "###morph_%zu_%u", i, uid);
    ...
    void Inspector::RenderLightProps(LightInfo& light)
    {
        ...
        if (ImGui::CollapsingHeader("Properties###lp_props", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            if (light.isSpot)
            {
                ...
                ImGui::PushItemWidth(-1);
                char said[32];
                snprintf(said, sizeof(said), "###sa_%u", uid);
                ...
                ImGui::PushItemWidth(-1);
                char iaid[32];
                snprintf(iaid, sizeof(iaid), "###ia_%u", uid);
    ...
    void Inspector::RenderMaterialProps(MaterialInfo& mat)
    {
        ...
        if (ImGui::CollapsingHeader("Properties###matp_props", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            char tmid[32];
            snprintf(tmid, sizeof(tmid), "###tm_%u", uid);
    ...
    void Inspector::RenderTextureProps(TextureInfo& tex)
    {
        ...
        if (ImGui::CollapsingHeader("Characteristics###txp_char", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            ImGui::SameLine();
            AttributeBadge(tex.hasAlpha ? "Yes" : "No", tex.hasAlpha);
            ImGui::NewLine();
            ImGui::Text("Is Cube: ");
            ImGui::SameLine();
            AttributeBadge(tex.isCube ? "Yes" : "No", tex.isCube);
            ImGui::NewLine();
    ...
    void Inspector::RenderAnimGroupProps(AnimGroupInfo& ag)
    {
        ...
        if (ImGui::CollapsingHeader("Controls###agp_ctrl", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            {
                EmitCmd(CMD_ANIM_PLAY, idx);
            }
            ImGui::SameLine();
            if (ImGui::Button("Pause"))
            {
                EmitCmd(CMD_ANIM_PAUSE, idx);
            }
            ImGui::SameLine();
            if (ImGui::Button("Stop"))
            {
                EmitCmd(CMD_ANIM_STOP, idx);
            }
            ...
            ImGui::PushItemWidth(-1);
            char fid[32];
            snprintf(fid, sizeof(fid), "###anim_frame_%u", idx);
    ...
    void Inspector::RenderPropertiesPanel(SceneData& data)
    {
        ...
        if (sel.type == EntityType::Camera && sel.index < data.cameras.size())
        {
            ...
            if (cam.uniqueId == sel.uniqueId)
            {
                RenderCameraProps(cam);
                return;
            }
        }
        if (sel.type == EntityType::Camera)
        {
            for (size_t i = 0; i < data.cameras.size(); i++)
                if (data.cameras[i].uniqueId == sel.uniqueId)
                {
                    sel.index = i;
                    RenderCameraProps(data.cameras[i]);
                    return;
                }
        }
        ...
        if (sel.type == EntityType::Node)
        {
            for (size_t i = 0; i < data.nodes.size(); i++)
                if (data.nodes[i].uniqueId == sel.uniqueId)
                {
                    sel.index = i;
                    RenderNodeProps(data.nodes[i]);
                    return;
                }
        }
        ...
        if (sel.type == EntityType::Mesh)
        {
            for (size_t i = 0; i < data.meshes.size(); i++)
                if (data.meshes[i].uniqueId == sel.uniqueId)
                {
                    sel.index = i;
                    RenderMeshProps(data.meshes[i], data);
                    return;
                }
        }
        ...
        if (sel.type == EntityType::Light)
        {
            for (size_t i = 0; i < data.lights.size(); i++)
                if (data.lights[i].uniqueId == sel.uniqueId)
                {
                    sel.index = i;
                    RenderLightProps(data.lights[i]);
                    return;
                }
        }
        ...
        {
            for (size_t i = 0; i < data.materials.size(); i++)
                if (data.materials[i].uniqueId == sel.uniqueId)
                {
                    sel.index = i;
                    RenderMaterialProps(data.materials[i]);
                    return;
                }
        }
        ...
        {
            for (size_t i = 0; i < data.textures.size(); i++)
                if (data.textures[i].uniqueId == sel.uniqueId)
                {
                    sel.index = i;
                    RenderTextureProps(data.textures[i]);
                    return;
                }
        }
        ...
        {
            for (size_t i = 0; i < data.animGroups.size(); i++)
                if (data.animGroups[i].index == sel.uniqueId)
                {
                    sel.index = i;
                    RenderAnimGroupProps(data.animGroups[i]);
                    return;
                }
        }
        ...
        {
            for (size_t i = 0; i < data.skeletons.size(); i++)
                if (data.skeletons[i].index == sel.uniqueId)
                {
                    sel.index = i;
                    RenderSkeletonProps(data.skeletons[i]);
                    return;
                }
        }
    ...
    void Inspector::RenderDebugPanel()
    {
        ...
        if (ImGui::CollapsingHeader("Texture Channels###dbg_tex"))
        {
            ...
            {
                char cid[32];
                snprintf(cid, sizeof(cid), "###tc_%d", i);
